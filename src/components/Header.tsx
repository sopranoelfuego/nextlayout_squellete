...
function Header({
  ...
  useEffect(() => {
    if (langue)
      setLanguage(() => listLanguages.find((lang) => lang.code === langue)!);
  }, [langue]);
    ...
    >
      ...
      <ClickAwayListener onClickAway={() => {
        ...
        <Box sx={{ position: "relative", marginRight: "2rem" }}>
          <Stack
            onClick={() => setDisplayLang((prev) => !prev)}
            sx={{
              padding: "0.3rem 0.5rem",
              transition: "all ease 400ms",
              borderRadius: "50px",
              border: "2px solid #dcf5f7",
              ":hover": { borderColor: "#62d6e3", cursor: "pointer" },
            }}
          >
          ...
          <Box
            sx={{
              display: displayLang ? "block" : "none",
              position: "absolute",
