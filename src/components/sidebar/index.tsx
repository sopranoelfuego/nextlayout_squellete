...
function Sidebar({ displayMenuDrawer, setDisplayMenuDrawer }: SidebarProps) {
  ...
  const handleCloseDisplayMenuDrawer=()=>setDisplayMenuDrawer((prev: boolean) => !prev)
  return (
    <>
      <Stack
      ...
      <Drawer
        ...
        onClose={handleCloseDisplayMenuDrawer}
      >
        <Box sx={{ width: "100%", minHeight: matches?"auto":"700px" }}>
          <Stack
            direction="row"
            spacing={{ xs: 1, sm: 2 }}
            position="relative"
          ...
          >
            {sidebarData.map((data: SidebarItemType) => (
              <SidebarItem item={data} expand={expand} key={data?.href} displayMenuDrawer={displayMenuDrawer} handleCloseDisplayMenuDrawer={handleCloseDisplayMenuDrawer}/>
            ))}
          </Box>
        </Box>
