...
function Sidebar({ displayMenuDrawer, setDisplayMenuDrawer }: SidebarProps) {
  ...
  const handleCloseDisplayMenuDrawer = () =>
    setDisplayMenuDrawer((prev: boolean) => !prev);
  return (
    <>
      <Stack
      ...
      >
        <Box sx={{ width: "100%", minHeight: "700px" }}>
          ...
          >
            {sidebarData
              .filter((data: SidebarItemType) =>
                data.priveleges.includes(user.role)
              )
              .map((data: SidebarItemType) => (
            ...
            >
              {sidebarData.map((data: SidebarItemType) => (
                <SidebarItem
                  ...
                  handleCloseDisplayMenuDrawer={handleCloseDisplayMenuDrawer}
                />
              ))}
            </Box>
          </Box>
